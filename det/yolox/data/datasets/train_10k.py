...
HB_BOP19_20_OBJS = [ref.cap_bottle_5k.id2obj[_i] for _i in [1, 2]]

SPLITS_HB_PBR = dict(
    train_10k=dict(
        name="train_10k",
        objs=ref.cap_bottle_5k.objects,  # all 33 objects
    ...
    train_pb_10k=dict(
        name="train_pb_10k",
        objs=HB_BOP19_20_OBJS,  # selected 16 objects for BOP19/20
        dataset_root='/home/rpm/Lab/cap_bottle/bc/data/pose_10k',
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
    ...
    test_pb_1k=dict(
        name="test_pb_1k",
        objs=HB_BOP19_20_OBJS,  # selected 16 objects for BOP19/20
        dataset_root='/home/rpm/Lab/cap_bottle/bc/data/pose_1k',
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
