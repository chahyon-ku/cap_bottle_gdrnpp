...
class HB_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "cap_bottle/train_10k"),
        )
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.cap_bottle_small.id2obj.items() if obj_name in self.objs]
        ...
        if data_cfg.get('name', 'train_10k') == 'train_10k':
            self.scenes = [f"{i:06d}" for i in range(1000)]
        elif 'pb' in data_cfg.get('name', 'train_10k'):
            self.scenes = ['000000']
        else:
            self.scenes = [f"{i:06d}" for i in range(100)]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.cap_bottle_small.obj2id[obj_name]:06d}.ply"),
...
hb_model_root = "cap_bottle/models/"
################################################################################

HB_BOP19_20_OBJS = [ref.cap_bottle_small.id2obj[_i] for _i in [1, 2]]

SPLITS_HB_PBR = dict(
    train_10k=dict(
        name="train_10k",
        objs=ref.cap_bottle_small.objects,  # all 33 objects
        dataset_root=osp.join(DATASETS_ROOT, "cap_bottle/train_10k"),
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
    ...
    test_1k=dict(
        name="test_1k",
        objs=ref.cap_bottle_small.objects,  # selected 16 objects for BOP19/20
        dataset_root=osp.join(DATASETS_ROOT, "cap_bottle/test_1k"),
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
    ...
    train_pb_10k=dict(
        name="train_pb_10k",
        objs=ref.cap_bottle_small.objects,  # all 33 objects
        dataset_root=osp.join(DATASETS_ROOT, "/home/rpm/Lab/cap_bottle/bc/data/pose_10k"),
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
    ...
    test_pb_1k=dict(
        name="test_pb_1k",
        objs=ref.cap_bottle_small.objects,  # selected 16 objects for BOP19/20
        dataset_root=osp.join(DATASETS_ROOT, "/home/rpm/Lab/cap_bottle/bc/data/pose_1k"),
        models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
...
# single obj splits
for obj in ref.cap_bottle_small.objects:
    for split in ["small_train"]:
        name = "hb_{}_{}".format(obj, split)
        if name not in SPLITS_HB_PBR:
            SPLITS_HB_PBR[name] = dict(
                name=name,
                objs=[obj],  # only this obj
                dataset_root=osp.join(DATASETS_ROOT, "cap_bottle/small_train"),
                models_root=osp.join(DATASETS_ROOT, "cap_bottle/models"),
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    MetadataCatalog.get(name).set(
        id="cap_bottle_small",  # NOTE: for pvnet to determine module
